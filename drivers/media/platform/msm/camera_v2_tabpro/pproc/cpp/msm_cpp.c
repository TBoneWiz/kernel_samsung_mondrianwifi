	...
	struct msm_camera_v4l2_ioctl_t *ioctl_ptr)
{
	...
	|| defined(CONFIG_MACH_KS01LGT) // QC debug patch for KS01
	pr_err("E, u_frame_info=%x\n", (uint32_t)u_frame_info);
#endif
...
#if defined(CONFIG_MACH_KS01SKT) || defined(CONFIG_MACH_KS01KTT)\
	|| defined(CONFIG_MACH_KS01LGT) // QC debug patch for KS01
	if (u_frame_info == NULL) {
		pr_err("u_frame_info is NULL\n");
		goto ERROR4;
	}
	if (ioctl_ptr == NULL) {
