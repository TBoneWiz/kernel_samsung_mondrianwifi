...
pr_info("crpalmer: this does test my change");
	config->fsg.nluns = 1;
	...
	if (dev->pdata && dev->pdata->cdrom) {
		...
		config->fsg.nluns++;
pr_info("crpalmer: did my device tree change actually work?");
	}
...
#if defined(CONFIG_SEC_LT03_PROJECT) || defined(CONFIG_MACH_MONDRIAN)
