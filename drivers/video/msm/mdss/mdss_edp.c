...
#ifdef CONFIG_MACH_PICASSO
	return edp_samsung_init_pwm_gpios(edp_drv);
#else
	return 0;
#endif
}
...
void mdss_edp_set_backlight(struct mdss_panel_data *pdata, u32 bl_level)
{
	...
	edp_samsung_set_backlight(edp_drv, bl_level);
#else
...
int mdss_edp_on(struct mdss_panel_data *pdata)
{
	...
	edp_samsung_edp_on_start(edp_drv);
#endif
...
#if defined(CONFIG_FB_MSM_EDP_SAMSUNG)
	edp_samsung_edp_on_end(edp_drv);
#endif
...
#if defined(CONFIG_FB_MSM_EDP_SAMSUNG)
	edp_samsung_edp_off_start(edp_drv);
#endif
...
#if defined(CONFIG_FB_MSM_EDP_SAMSUNG)
	edp_drv->panel_data.event_handler = edp_samsung_event_handler;
#else
...
#if defined(CONFIG_FB_MSM_EDP_SAMSUNG)
			edp_samsung_event_link_train(ep);
#else
